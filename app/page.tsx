// app/page.tsx — Writing (Typing + Paper Mode), time selector (30/40m), rubric feedback
"use client";
...
const QUESTION_COUNT: Record<Exclude<Subject, "writing">, number> = {
  maths: 12, english: 10, vr: 10, nvr: 8, reasoning: 12
};
const QUIZ_SECONDS = 10 * 60;
const DAILY_CAP_SECONDS = 30 * 60;

// Default writing time (Kent ~40m incl. planning). We allow quick selection 30m or 40m.
  ...
  if (opt.kind === "arrow") {
    return (
      <svg width={size} height={size} viewBox="0 0 100 100" style={{ transform:`rotate(${opt.rotation ?? 0}deg)` }}>
        <defs><marker id="arrowhead" markerWidth="10" markerHeight="7" refX="0" refY="3.5" orient="auto"><polygon points="0 0, 10 3.5, 0 7" fill={fill} stroke={stroke}/></marker></defs>
        <line x1="10" y1="50" x2="80" y2="50" stroke={stroke} strokeWidth="8" markerEnd="url(#arrowhead)" />
      </svg>
    );
  }
  if (opt.kind === "dots") {
    const n = opt.count ?? 3, r = 6, gap = 20, cols = 4;
    return (
...
function genNvrSvg(count:number): Question[]{
  ...
  for(let i=0;i<count;i++){
    ...
    if(mode==="fill"){
      ...
      out.push({ id:`nvr-fill-${i}-${common}`, subject:"nvr", stem:"Which is the odd one out? (by shading)", choices:["A","B","C","D"], answerIndex: order.indexOf(3), svgOptions: order.map(x=>options[x]), explanation:`Three are ${common}-filled; one is ${odd}.` });
    } else if(mode==="rotation"){
      const rot=pick([0,90,180,270]); const odd=(rot+45)%360;
      const options=[
      ...
      const order=shuffle([0,1,2,3]);
      out.push({ id:`nvr-rot-${i}-${rot}`, subject:"nvr", stem:"Which arrow is different? (rotation)", choices:["A","B","C","D"], answerIndex: order.indexOf(3), svgOptions: order.map(x=>options[x]), explanation:`Three at ${rot}°, one at ${odd}°.` });
    } else if(mode==="size"){
      const size=pick([56,64,72]); const odd=size+16; const shape=pick(shapes);
      const options=[
      ...
      const order=shuffle([0,1,2,3]);
      out.push({ id:`nvr-size-${i}-${shape}`, subject:"nvr", stem:"Which is the odd one out? (by size)", choices:["A","B","C","D"], answerIndex: order.indexOf(3), svgOptions: order.map(x=>options[x]), explanation:"One is larger than the other three." });
    } else {
      const common=pick([3,4,5]); const odd=common+2;
      const options=[
      ...
      const order=shuffle([0,1,2,3]);
      out.push({ id:`nvr-count-${i}-${common}`, subject:"nvr", stem:"Which set has a different number of dots?", choices:["A","B","C","D"], answerIndex: order.indexOf(3), svgOptions: order.map(x=>options[x]), explanation:`Three show ${common} dots; one shows ${odd}.` });
    }
  }
  return out;
}
...
function genMathGeometry(count:number): Question[]{
  ...
  for(let i=0;i<count;i++){
    ...
    if(kind==="rect-area"){
      const w=randInt(3,12), h=randInt(3,12), ans=w*h;
      ...
      const b=randInt(4,12), h=randInt(3,10), ans=0.5*b*h, ansStr=String(Number.isInteger(ans)?ans:Math.round(ans*10)/10);
      ...
      const w=randInt(3,12), h=randInt(3,12), ans=2*(w+h);
      ...
      const a=randInt(3,10), b=randInt(4,11), c=randInt(5,12), ans=a+b+c;
      ...
      const known=randInt(40,140), ans=180-known;
      ...
      const a=randInt(30,80), b=randInt(30,80), ans=180-a-b;
      ...
      const shape=pick(["square","rectangle","isosceles triangle"]); let ans=shape==="square"?4:shape==="rectangle"?2:1;
...
function MathsDiagram({ qid }: { qid: string }) {
  ...
  if (qid.startsWith("math-triA-")) {
    const parts = qid.split("-"); const b = parseInt(parts[3], 10), h = parseInt(parts[4], 10);
    return (
  ...
  if (qid.startsWith("math-triP-")) {
    const parts = qid.split("-"); const a = parseInt(parts[3],10), b = parseInt(parts[4],10), c = parseInt(parts[5],10);
    return (
      <svg width="240" height="170" viewBox="0 0 240 170">
  ...
  if (qid.startsWith("math-angT-")) {
    const parts = qid.split("-"); const a = parseInt(parts[3],10), b = parseInt(parts[4],10);
    return (
      <svg width="240" height="170" viewBox="0 0 240 170">
...
function sentences(s: string){ return (s.trim().match(/[^.!?]+[.!?]+/g) || []).map(x=>x.trim()); }
...
function estimateReadingAge(s: string){
  ...
  const sentencesN=sentences(s).length || 1;
...
function assessWriting(s: string, prompt?: WritingPrompt): WritingReport {
  const w=countWords(s), sens=sentences(s).length, ttr=typeTokenRatio(s), ra=estimateReadingAge(s);
  const spellingFlags = (s.match(/\b(i)\b/g)||[]).length; // naive example: isolated "i" lowercase
  ...
  if(sens<6){ notes.push("Try more sentences and vary openings/connectives."); }
  else { notes.push("Nice number of sentences; vary lengths for flow."); score+=1; }
  if(ttr>0.5){ notes.push("Great variety of vocabulary."); score+=2; }
  else if(ttr>0.4){ notes.push("Decent word variety."); score+=1; }
  else { notes.push("Try not to repeat words; use precise vocabulary."); }
  if(spellingFlags>0){ notes.push("Watch out for capital 'I' — it should be uppercase when used alone."); }
  notes.push("Make sure you have a clear beginning, middle and end.");
  if(prompt?.kent_style){ notes.push("Stay focused on the moment/description (typical Kent prompt)."); }
...
export default function Page(){
  ...
  function answer(choiceIndex:number){
    ...
    if(index+1<questions.length) setIndex(i=>i+1); else finishNow();
  }
  function finishNow(){
  ...
  const writingMetrics = useMemo(()=>{
    return { words: countWords(writing), sentences: sentences(writing).length, readingAge: estimateReadingAge(writing) };
  }, [writing]);

  const writingReport = useMemo(()=> mode==="writingResults" ? assessWriting(writing, writingPrompt||undefined) : null, [mode, writing, writingPrompt]);
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", display:"grid", gap:16 }}>
          ...
          <Card>
            <div style={{ display:"grid", gap:12 }}>
              ...
              {(current.id.startsWith("math-rect") || current.id.startsWith("math-tri") || current.id.startsWith("math-ang")) && (
                <div style={{ display:"grid", placeItems:"center", padding:8 }}><MathsDiagram qid={current.id} /></div>
              )}
          ...
          <Card>
            <div style={{ display:"grid", gap:16 }}>
              ...
              <div style={{ display:"grid", gap:8 }}>
                {questions.map((q,i)=>{
                  const a=answers[i]; const verdict=a?.correct?"✅ Correct":"❌ Incorrect";
                  return (
                    <div key={q.id} style={{ border:"2px solid #6f9e63", borderRadius:10, padding:12, background:"#eef7ea" }}>
                      ...
                      {(q.id.startsWith("math-rect") || q.id.startsWith("math-tri") || q.id.startsWith("math-ang")) && (<div style={{ padding:"8px 0" }}><MathsDiagram qid={q.id} /></div>)}
        ...
        {mode==="writingResults" && writingReport && (
          <Card>
            <div style={{ display:"grid", gap:16 }}>
              <div style={{ display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
                <div style={{ width:80, height:80, borderRadius:"50%", background:"#9ad27a", border:"4px solid #2f4f2f", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:800 }}>
                  {writingReport.band}
                </div>
                <div>
              ...
              <ul style={{ marginLeft:18 }}>
                {writingReport.notes.map((n,i)=>(<li key={i} style={{ marginBottom:6 }}>{n}</li>))}
              </ul>
