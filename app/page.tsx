// app/page.tsx — 11+ Adventure (Minecraft-styled)
// - Subjects: Maths, English, VR, NVR, Comprehension, Writing (typed only, 30m)
// - Features: 10-min quiz timer, 30-min daily cap, pause/end, unique questions per quiz
// - Generators: Maths (geometry/angles/symmetry), English (synonyms/prefixes/suffixes/homophones/grammar/cloze),
//               VR sequences, NVR (odd-one-out via SVG), Comprehension (from JSON file)
// - Data bank: optionally loads /questions/{math,english,vr,nvr}.json
// - Comprehension file format: public/questions/comprehension.json  ->  Array<Passage>
//   [{ id, title?, text, questions: [{ stem, choices, answerIndex, explanation? }] }]
// - Removed: Writing (paper) + 40m option
...
type Question = {
  id: string;
  subject: Exclude<Subject, "comprehension" | "writing"> | "comprehension";
  stem: string;
  choices: string[];
  answerIndex: number;
  explanation?: string;
...
/** ------------------- Config ------------------- */
const QUESTION_COUNT: Record<Exclude<Subject, "comprehension" | "writing">, number> = {
  maths: 12,
  english: 12,
  vr: 10,
  nvr: 8,
};
const COMP_QUESTION_COUNT = 5; // per passage (cap if passage has fewer)
...
async function loadComprehension(path: string): Promise<Passage[]> {
...
type WritingReport = { score: number; band: "Developing" | "Secure" | "Strong"; notes: string[]; metrics: { words: number; sentences: number; ttr: number; readingAge: number } };
function assessWriting(s: string): WritingReport {
  const w = countWords(s); const sens = sentencesList(s).length; const ttr = typeTokenRatio(s); const ra = estimateReadingAge(s);
  ...
  return { score, band, notes, metrics: { words: w, sentences: sens, ttr, readingAge: ra } };
}
...
export default function Page() {
  ...
  const [writingTime, setWritingTime] = useState<number>(WRITING_SECONDS);
  ...
  function buildQuizSet(subj: Exclude<Subject, "comprehension" | "writing">, count: number): Question[] {
    if (subj === "english") {
  ...
  async function startComprehension() {
    if (!canStart) return;
    const passages = await loadComprehension("/questions/comprehension.json");
    if (!Array.isArray(passages) || passages.length === 0) return;
    const chosen = passages[Math.floor(Math.random() * passages.length)];
  ...
  function startQuiz(subj: Subject) {
    ...
    if (subj === "writing") {
      ...
      setWriting("");
      setWritingPrompt(p);
      setWritingTime(Math.min(WRITING_SECONDS, remainingToday));
      setPaused(false);
      setMode("writing");
      return;
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,#c8e6c9,#a5d6a7)", color: "#20351f", padding: 24, boxSizing: "border-box" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto", display: "grid", gap: 16 }}>
        ...
        <div style={{ opacity: 0.8, fontSize: 14, marginBottom: 8 }}>Minecraft-inspired • Kent/Bexley • 10-minute quizzes • 30-minute daily cap • Writing (typed)</div>
          ...
          <Card>
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                ...
                <BlockButton disabled={!canStart} onClick={() => startQuiz("comprehension")} style={{ background: "#9ad27a" }}>Comprehension</BlockButton>
                <BlockButton disabled={!canStart} onClick={() => startQuiz("writing")} style={{ background: "#9ad27a" }}>Writing (typed, 30m)</BlockButton>
              </div>
              <div style={{ fontSize: 12, opacity: 0.75 }}>
                Question counts — Maths: {QUESTION_COUNT.maths} · English: {QUESTION_COUNT.english} · VR: {QUESTION_COUNT.vr} · NVR: {QUESTION_COUNT.nvr} · Comprehension: up to {COMP_QUESTION_COUNT}
              </div>
        ...
        {mode === "writingResults" && writingReport && (
          <Card>
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <BlockButton onClick={() => setMode("menu")}>Back to Menu</BlockButton>
                <BlockButton onClick={() => startQuiz("writing")}>Try another Writing prompt</BlockButton>
              </div>
            </div>
          </Card>
        )}
        ...
        {mode === "practice" && (
          <Card>
            <div style={{ display: "grid", gap: 12 }}>
              <div style={{ fontSize: 18, fontWeight: 800 }}>Practice Hub (links)</div>
              <div style={{ opacity: 0.8, fontSize: 14 }}>Open-licensed content can be added to your local question bank with attribution. Proprietary materials should be used via links.</div>
